...
export function schemaLookup(schemaName: string) {
  // dummy value to simulate lookup from store
  ...
  const insolvSchema: Schema = {
    ...
    attributes: [
      ...
      {
        label:
          "Written demand has been made to company's registered office, and the debtor has failed to pay, secure or compound the sum within 3 weeks",
        type: AttributeType.Boolean,
      },
      {
        label: "Judgment has been issued in favor of a creditor, which has not been fully complied with",
        type: AttributeType.Boolean,
      },
