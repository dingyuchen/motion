import { AttributeType, Model } from "../../motion-bee/lib/types";
...
export function ModelCard(props: any) {
  ...
  for (let attribute of model.attributes) {
    if (attribute.type === AttributeType.Collection) {
      ...
      const subSchemaLabel: string = schema.attributes.find(
        (schemaAttr: AttributeDefinition) => schemaAttr.label === attribute.label
      ).subtype;
    ...
    if (attribute.type === AttributeType.Boolean) {
      children.push(
        <div className="flex mt-2">
          ...
          <select
            className="border-2 h-12 pl-2 text-lg py-2"
    ...
    if (attribute.type === AttributeType.Number) {
      children.push(
        <div className="flex mt-2">
          ...
          <input
            type="number"
            className="border-2 h-12 pl-4 text-lg py-2"
            onChange={handleChange}
            name={attribute.label}
            value={attribute.value}
          />
    ...
    if (attribute.type === AttributeType.Enum) {
      const enumSet = schema.attributes.find(
        (schemaAttr: AttributeDefinition) => schemaAttr.label === attribute.label
      ).enumSet;
...
const Collection = (props: any) => {
    ...
    children.push(
      <div className="flex" key={i}>
        <div className="text-lg w-24 border-2 flex flex-col justify-center items-center">
          {i + 1}
          <div
            className="border-2 hover:bg-red-600 hover:text-white cursor-pointer text-sm py-2 px-2 mt-10"
            onClick={() => deleteSubModel(i)}
          >
            Delete
          </div>
        </div>
  ...
  return (
    <div className="border-2 p-4 my-4">
      ...
      <div
        className="addSubModel border-2 mt-6 w-max px-4 py-4 cursor-pointer hover:bg-indigo-600 hover:text-white
